...
[ "$YES_OR_NO" = "y" ] && {
    L1="[INFO][`date +%Y-%m-%d` `date +%H:%M:%S`]nodebb on Ubuntu fast shell started-up"
	L2="[IMPORTANT]Please ensure apt is not in use by others processes,or install will be failed"
	L3="[INFO][`date +%Y-%m-%d` `date +%H:%M:%S`]Starting get parameter"
	...
	L6="[INFO][`date +%Y-%m-%d` `date +%H:%M:%S`]sources set up successfully,bach up is in /root"
...
[ "$YES_OR_NO" = "n" ] && {
	L1="[INFO][`date +%Y-%m-%d` `date +%H:%M:%S`]nodebb on Ubuntu快速设置脚本启动"
	L2="[重要]请确保apt没有被占用，否则将造成无法安装"
	L3="[INFO][`date +%Y-%m-%d` `date +%H:%M:%S`]开始获取参数"
	L4="您的服务器在大陆还是海外？这无关紧要，但是影响速度\e[49m"
	L5="请使用M/O，M表示大陆，O表示海外；如果您想使用官方的下载，请输入O"
	L6="[INFO][`date +%Y-%m-%d` `date +%H:%M:%S`]软件源设置成功，旧的源备份在/root下"
